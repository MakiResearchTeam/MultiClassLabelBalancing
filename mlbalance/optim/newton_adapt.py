...
class NewtonAdaptive(Optimizer):
    def __init__(self, it=500, hess_update_period=1.5, eig_correction=2.0, step_size=1.0, mu=0.5, print_period=-1):
        """
        Differs from Newton in `newton.py` only in the way it updates hessian. In this implementation hessian updates
        are significantly rarer which leads to performance improvement at the cost of slight accuracy drop.
        ...
        hess_update_period : int
            Hessian updates after every N iterations. N is recomputed on each hessian update as
            N = N * hess_update_period.
    ...
    def compute_update(self, x, grad_fn, hess_fn, it: int = 0):
        if it % int(self.update_period) == 0 or self.inv_hessian_cache is None:
            hess = hess_fn(x)
            ...
            self.update_period = self.update_period * self.hess_update_period
